...
logger = get_logger('chainrisk.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if config.get("scenario_type") == "cape":
            return cls._start_cape_simulation(simulation_id=simulation_id, config=config)
    ...
    @classmethod
    def _start_cape_simulation(cls, simulation_id: str, config: Dict[str, Any]) -> SimulationRunState:
        t_max = int(config.get("t_max", 0))
        if t_max <= 0:
            raise ValueError("CAPE simulation requires config.t_max > 0")

        state = SimulationRunState(
            simulation_id=simulation_id,
            runner_status=RunnerStatus.RUNNING,
            total_rounds=t_max,
            total_simulation_hours=t_max,
            started_at=datetime.now().isoformat(),
        )
        cls._save_run_state(state)

        try:
            env = CAPEEnvironment(config=config)
            env.run()
            state.current_round = t_max
            state.simulated_hours = t_max
