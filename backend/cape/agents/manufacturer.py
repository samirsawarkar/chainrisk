...
class ManufacturerAgent(CAPEAgent):
    def _decide(self, tick: int, state: dict) -> list:
        ...
        for sku_id, inv in inventory.items():
            ...
            if observed_downstream > 0:
                ...
                amplification_factor = float(self.config.get("amplification_factor", 1.55))
            ...
            if qty > 0:
                ...
                order_event = OrderEvent(
                    event_id=self._event_id(tick=tick, event_type="OrderEvent", target_node=supplier_node, sku_id=sku_id, quantity=qty),
                    tick=tick,
                    source_node=self.node_id,
        ...
        events.append(
            CapacityEvent(
                event_id=self._event_id(tick=tick, event_type="CapacityEvent", target_node="SYSTEM", sku_id="ALL", quantity=int(capacity_consumed)),
                tick=tick,
                source_node=self.node_id,
                target_node="SYSTEM",
                sku_id="ALL",
                capacity_used=int(capacity_consumed),
    ...
    def _order_up_to_level(self, sku_id: str, capacity_avail: int, backlog: int = 0) -> int:
        ...
        avg_d = (sum(demand_hist) / len(demand_hist)) if demand_hist else float(self.config.get("avg_demand", 10))
        amplification = 1.0 + min(1.2, max(0.0, backlog / max(1.0, avg_d)))
