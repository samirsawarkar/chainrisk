...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    """
     OASIS  LLM(Gemini, OpenAI ).

    :GEMINI_API_KEY,GEMINI_API_BASE_URL(),
    GEMINI_MODEL_OASIS_PARALLEL_A / GEMINI_MODEL_OASIS_PARALLEL_B().

    Args:
        config: ( llm_model )
        use_boost: True  B ( Reddit), A( Twitter)
    """
    ...
    if use_boost:
        llm_model = (
            os.environ.get("GEMINI_MODEL_OASIS_PARALLEL_B")
            or config.get("llm_model")
            or Config.GEMINI_MODEL_OASIS_PARALLEL_B
        )
        config_label = "[Gemini OASIS-B]"
    else:
        llm_model = (
            os.environ.get("GEMINI_MODEL_OASIS_PARALLEL_A")
            or config.get("llm_model")
            or Config.GEMINI_MODEL_OASIS_PARALLEL_A
        )
        config_label = "[Gemini OASIS-A]"

    base_preview = (Config.GEMINI_API_BASE_URL or "")[:48]
    print(f"{config_label} model={llm_model}, base_url={base_preview}...")
