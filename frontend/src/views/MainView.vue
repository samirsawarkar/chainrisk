  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">CHAINRISK</div>
      </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value =
      'No upload in memory. Go to Home, add PDF/MD/TXT files and a simulation brief, then click Start.'
    addLog(
      'Error: No pending files for /process/new (refresh clears memory; start again from Home).'
    )
    return
  }
  ...
  try {
    ...
    if (err.response?.data?.traceback) {
      console.error('Ontology API traceback:', err.response.data.traceback)
    }
